...
impl CudaDevice {
    ...
    pub fn get_or_load_func(&self, fn_name: &str, mdl: &kernels::Module) -> Result<CudaFunc> {
        ...
        // A cubin matches only its exact architecture; anything else falls
        // through to the PTX, which the driver translates. Only a cubin loads
        // when the driver predates the build toolkit's PTX ISA.
        let image = match self
            .context
            .compute_capability()
            .ok()
            .and_then(|(major, minor)| {
                let architecture = u32::try_from(major * 10 + minor).ok()?;
                mdl.cubins()
                    .iter()
                    .find(|cubin| cubin.architecture == architecture)
            }) {
            Some(cubin) => cudarc::nvrtc::Ptx::from_binary(cubin.image.to_vec()),
            None => mdl.ptx().into(),
        };
