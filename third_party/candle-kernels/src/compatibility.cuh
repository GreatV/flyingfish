...
#include "cuda_bf16.h"
#include "cuda_fp8.h"
// flyingfish patch: `CUDA_VERSION` is needed by the `__hmax_nan` guard below
// and the fp16/bf16/fp8 headers do not provide it. `cuda.h` does, and it is
// the only header that does in a device-only (`--ptx`) compilation.
#include <cuda.h>
...
// flyingfish patch: `&& CUDA_VERSION < 12060`.
//
// Upstream assumes the toolkit declares `__hmax_nan`/`__hmin_nan` only for
// sm_80 and later, so it supplies them below that. CUDA 12.6 changed the
// strategy: `cuda_fp16.hpp` now declares both unconditionally and selects the
// implementation internally with `NV_IF_ELSE_TARGET(NV_PROVIDES_SM_80, ...)`.
// Against any such toolkit this block is a redefinition, so compiling for a
// pre-Ampere target fails outright:
//
//   error: function "__hmax_nan(__half, __half)" has already been defined
//
// Verified with CUDA 12.6, 12.8 and 13.2: all three declare it unconditionally,
// and the failure reproduces at `--gpu-architecture=compute_75` on every one.
// It is invisible on sm_80+ because the guard already excludes the block there,
// which is why every Ampere-or-later host builds this crate cleanly.
#if __CUDA_ARCH__ < 800 && CUDA_VERSION < 12060
